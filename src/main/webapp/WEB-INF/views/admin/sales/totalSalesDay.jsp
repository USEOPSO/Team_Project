...
<script type="text/javascript">
	...
	function selectYear() {
		...
		window.location='/admin/totalSalesDay?year=' + year;
	}
							...
							<tr>
								<c:forEach var="dts" items="${dtslist}" varStatus="status">
									...
									<c:if test="${status.first}">
										<%-- 현재 (월) 설정 --%>
										<c:set var="date" value="${dts.paymentMonth}" />
									</c:if>
									<c:if test="${dts.paymentWay ne null}">
										<c:if test="${dts.paymentWay eq '카드'}">
											<td align="center">
											 ...
											 <td align="center">
												 ...
												 <input type="hidden" name="cardTotal" value="${dts.total}">
												<fmt:formatNumber value="${dts.total}" pattern="#,###원"/>
											</td>
										</c:if>
										
										<c:if test="${dts.paymentWay eq '현금'}">
											<c:if test="${card == 0}">
												<td align="center">
													<input type="hidden" name="date" value="${dts.paymentMonth}-${dts.paymentDay}">
													${dts.paymentMonth}월 ${dts.paymentDay}일
											 ...
											 <td align="center">
												 ...
												 <input type="hidden" name="cashTotal" value="${dts.total}">
												<fmt:formatNumber value="${dts.total}" pattern="#,###원"/>
											</td>
										</c:if>
										
										<c:if test="${dts.paymentWay eq '카카오페이'}">
											<c:if test="${card == 0}">
												<td align="center">
													<input type="hidden" name="date" value="${dts.paymentMonth}-${dts.paymentDay}">
													${dts.paymentMonth}월 ${dts.paymentDay}일
											...
											<td align="center">
												 ...
												 <input type="hidden" name="kakaoTotal" value="${dts.total}">
												<fmt:formatNumber value="${dts.total}" pattern="#,###원"/>
											</td>
										</c:if>
									</c:if>
									<c:if test="${dts.paymentWay eq null}">
										<%-- 카드가 없는 경우 --%>
										<c:if test="${card == 0}">
											<td align="center">
												<input type="hidden" name="date" value="${dts.paymentMonth}-${dts.paymentDay}">
										...
										<td align="center">
											<input type="hidden" name="allTotal" value="${dts.total}">
											<c:set var="total" value="${total + dts.total}" />
											<fmt:formatNumber value="${dts.total}" pattern="#,###원"/>
										</td>
