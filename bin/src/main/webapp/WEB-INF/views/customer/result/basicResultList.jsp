				...
				<div style="padding-top: 50px">
					<table class="table table-hover">
						...
						<c:forEach var="dto" items="${list}">
							<tr>
								<td align="center"><a href="basicResult?reserveNo=${dto.reserveNo}">${dto.reserveNo}</a></td>
						...
						<tr>
							<td align="center" colspan="11">
						    	<c:if test="${paging.startPage > 10}">
									<a href="/customer/basicResultList?pageNum=${paging.prev}">[이전]</a>
								</c:if>
							
								<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
									<a href="/customer/basicResultList?pageNum=${num}">${num}</a>
								</c:forEach>
							
								<c:if test="${paging.endPage < paging.pageCount}" >
									<a href="/customer/basicResultList?pageNum=${paging.next}">[다음]</a>
								</c:if>
							</td>
		   			  	</tr>
					</table>
